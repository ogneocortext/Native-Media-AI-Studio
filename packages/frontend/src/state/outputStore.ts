...
interface OutputState {
  ...
  fetchByType: (type: "images" | "video" | "audio" | "3d") => Promise<void>;
  ...
  fetchDuplicates: (quick?: boolean) => Promise<
    Array<{
      hash: string;
      count: number;
      size_bytes: number;
...
export async function fetchRecentOutputsFromAPI(limit = 10): Promise<OutputFile[]> {
  const base = await getBackendUrl();
  const res = await fetchWithTimeout(base + "/api/outputs/recent?limit=" + limit, {
    timeout: 30000,
  });
...
export async function fetchOutputsByTypeFromAPI(
  ...
  const res = await fetchWithTimeout(
    base + "/api/outputs?file_type=" + fileType + "&limit=" + limit,
    { timeout: 30000 },
  );
...
export const useOutputStore = create<OutputState>()((set, get) => ({
  ...
  fetchOutputs: async () => {
    ...
    try {
      ...
      const response = await fetchOutputsFromAPI(fileType, filter.limit || 200, filter.offset, {
  ...
  fetchRecent: async (limit = 10) => {
    ...
    try {
      ...
      set({
        error: error instanceof Error ? error.message : "Failed to fetch recent outputs",
      });
    } finally {
      set({ isLoading: false });
    }
  ...
  fetchByType: async (type) => {
    ...
    try {
      ...
      set({
        error: error instanceof Error ? error.message : "Failed to fetch " + type + " outputs",
      });
    } finally {
      set({ isLoading: false });
    }
  ...
  deleteOutput: async (relativePath: string) => {
    ...
    const res = await fetchWithTimeout(base + "/api/outputs/" + encoded, {
      method: "DELETE",
      timeout: 30000,
    });
    ...
    set((state) => ({
      ...
      recentOutputs: state.recentOutputs.filter((o) => o.relative_path !== relativePath),
    }));
  },
  ...
  renameOutput: async (relativePath: string, newName: string) => {
    const base = await getBackendUrl();
    const encoded = relativePath.split("/").map(encodeURIComponent).join("/");
    const res = await fetchWithTimeout(base + "/api/outputs/" + encoded + "/rename", {
      method: "POST",
  ...
  bulkDelete: async (paths: string[]) => {
    const base = await getBackendUrl();
    const res = await fetchWithTimeout(base + "/api/outputs/bulk-delete", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  fetchDuplicates: async (quick = true) => {
    const base = await getBackendUrl();
    const res = await fetchWithTimeout(base + "/api/outputs/duplicates/groups?quick=" + quick, {
      timeout: 30000,
    });
