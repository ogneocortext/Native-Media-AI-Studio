...
export async function fetchRecentOutputsFromAPI(
  limit = 10,
): Promise<OutputFile[]> {
  const base = await getBackendUrl();
...
export const useRecentOutputs = () =>
  useOutputStore((state) => state.recentOutputs);

/** Subscribe to selected output only */
export const useSelectedOutput = () =>
  useOutputStore((state) => state.selectedOutput);
...
export const useOutputLoading = () =>
  useOutputStore((state) => state.isLoading);
