...
export function ThreeJSStudio() {
  ...
  const createMeshForObject = useCallback(
    async (obj: AnimObject, THREE: any) => {
  ...
  const generatedSceneUpdateRef = useRef<
  ...
  useEffect(() => {
    ...
    const initScene = async () => {
      ...
      const animate = () => {
        ...
        if (isPlay) {
          characterMixersRef.current.forEach((mixer) => {
            mixer.update(delta);
          });
        }
        if (
  ...
  useEffect(() => {
    if (
      ...
      !libraryTracks.some((t) => t.filename === selectedTrack)
    ) {
      setSelectedTrack("");
  ...
  useEffect(() => {
    ...
    const load = (fileType: string) =>
      fetch(`/api/outputs?file_type=${fileType}&limit=12`)
        ...
        .catch(() => null);
    Promise.all([load("audio"), load("video"), load("image")]).then(
      ([a, v, i]) => {
        if (cancelled) return;
    ...
    >
      ...
      >
          ...
          >
            ...
            {!tracksLoading &&
              ...
              libraryTracks.map((t) => {
                ...
                const metaStr =
                  meta?.bpm && meta?.duration
                    ? ` (${meta.bpm} BPM, ${meta.duration}s)`
                    : "";
        ...
        {selectedTrack && (
          <audio
            ref={audioElementRef}
            src={`/api/audio/file/${encodeURIComponent(selectedTrack)}`}
      ...
      >
        ...
        {codePanelOpen && (
          <div
            className={`code-panel absolute top-2 left-2 w-80 max-h-[calc(100%-1rem)] z-10 flex flex-col gap-2 ${focusMode ? "hidden" : ""}`}
          >
