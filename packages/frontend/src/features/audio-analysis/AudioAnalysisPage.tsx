          ...
          {analysis && (
            <div className={DS.section}>
              ...
              {analysis.beat_times && analysis.beat_times.length > 0 && (
                <div className={DS.card}>
                  <h3 className={DS.sectionTitle}><Zap size={14} />Beat Timeline<span className={DS.textXs}>({analysis.beat_times.length} beats)</span></h3>
                  <div className={DS.flexWrap}>{analysis.beat_times.slice(0, 100).map((t, i) => (<span key={i} className={DS.textXs}>{t.toFixed(2)}</span>))}{analysis.beat_times.length > 100 && <span className={DS.textXs}>+{analysis.beat_times.length - 100} more</span>}</div>
                </div>
              )}
            </div>
          )}
