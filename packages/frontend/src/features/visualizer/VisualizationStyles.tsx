...
export function AudioReactiveCore({
  ...
  useFrame((s) => {
    ...
    if (!sceneFrozen)
      meshRef.current.rotation.z = t * 0.02 * vizParams.rotationSpeed;
...
export function FrequencyRings({
  ...
  useFrame((s) => {
    ...
    if (!sceneFrozen)
      rotRef.current +=
        0.004 * vizParams.rotationSpeed * (1 + features.energy * 2);
...
export function PulseRings({ audioData, vizParams, sceneFrozen }: VizProps) {
  ...
  useFrame((s) => {
    ...
    if (!sceneFrozen)
      groupRef.current.rotation.y = t * 0.02 * vizParams.rotationSpeed;
  });
...
export function SpectrumBars({ audioData, vizParams, sceneFrozen }: VizProps) {
  ...
  useFrame((_s) => {
    ...
    if (!sceneFrozen)
      rotRef.current += 0.004 * vizParams.rotationSpeed * (1 + peak);
    ...
    if (discRef.current) {
      if (!sceneFrozen)
        discRef.current.rotation.y =
          t * 0.5 * vizParams.rotationSpeed * (1 + bass);
...
export function AuroraRibbon({ audioData, vizParams, sceneFrozen }: VizProps) {
  ...
  useFrame((s) => {
    ...
    if (!sceneFrozen)
      meshRef.current.rotation.z = t * 0.01 * vizParams.rotationSpeed;
  });
...
export function OceanWaves({ audioData, vizParams, sceneFrozen }: VizProps) {
  ...
  useFrame((s) => {
    ...
    if (!sceneFrozen)
      meshRef.current.rotation.z = t * 0.005 * vizParams.rotationSpeed;
  });
...
export function FractalViz({ audioData, vizParams, sceneFrozen }: VizProps) {
  ...
  useFrame((s) => {
    ...
    beatPulse.current *= 0.9;
    if (!sceneFrozen)
      rotRef.current += 0.005 * vizParams.rotationSpeed * (1 + bass * 2);
    ...
    ringRefs.current.forEach((ring, i) => {
      ...
      ring.rotation.y =
        rotRef.current * (i + 1) * 0.2 + Math.cos(t * 0.7 + i) * mid * 0.5;
...
export function StormViz({ audioData, vizParams, sceneFrozen }: VizProps) {
  ...
  useFrame(() => {
    ...
    if (!sceneFrozen)
      rotRef.current += 0.006 * vizParams.rotationSpeed * (1 + treble * 3);
