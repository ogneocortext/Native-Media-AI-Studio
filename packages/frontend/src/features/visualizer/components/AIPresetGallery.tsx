...
export function AIPresetGallery({ onApplyPreset, refreshKey }: AIPresetGalleryProps) {
  ...
  const fetchPresets = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetchWithTimeout("/api/integrations/visualization-presets", {
        timeout: 30000,
      });
      if (res.ok) {
      ...
      const res2 = await fetchWithTimeout("/api/integrations/ollama/visualizer/presets", {
        timeout: 5000,
      }).catch(() => null);
      if (res2?.ok) {
  ...
  const handleDelete = useCallback(async (p: SavedPreset) => {
    ...
    try {
      const res = await fetchWithTimeout(`/api/integrations/ollama/visualizer/preset/${p.id}`, {
        method: "DELETE",
        timeout: 30000,
      });
      if (res.ok) {
    ...
    <div className="viz-gallery">
      <div className="viz-gallery-header">
        <div className="viz-gallery-title">
          ...
          <span className="viz-gallery-count">
          ...
          return (
            ...
            >
              ...
              <div className="viz-gallery-card-actions">
                ...
                <button
                  className="viz-gallery-icon danger"
