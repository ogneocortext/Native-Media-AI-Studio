#!/usr/bin/env python3
"""Refactor HyperFrames monolithic compositions into sub-compositions."""
...
# Read inline DATA from original files
...
root_data = extract_data(os.path.join(BASE, "index.html"))
v3_data = extract_data(os.path.join(V3_BASE, "index.html"))

# Shared helpers template
HELPERS_JS = """
    // ─── SHARED HELPERS ────────────────────────────────────────────────────
    window._seed = (window._seed !== undefined) ? window._seed : 42;
...
# Canvas effects sub-comp (stars + particles + waveform)
CANVAS_STYLES = """
    #starCanvas {{
...
# Build files
comps = {
    "backgrounds.html": (BG_STYLES, BG_MARKUP, BG_SCRIPT, "0"),
    "canvas-effects.html": (CANVAS_STYLES, CANVAS_MARKUP, CANVAS_SCRIPT, "4"),
    "overlays.html": (OVERLAYS_STYLES, OVERLAYS_MARKUP, OVERLAYS_SCRIPT, "5"),
    "orb.html": (ORB_STYLES, ORB_MARKUP, ORB_SCRIPT, "9"),
    "spectrum.html": (SPECTRUM_STYLES, SPECTRUM_MARKUP, SPECTRUM_SCRIPT, "11"),
    "lyrics.html": (LYRICS_STYLES, LYRICS_MARKUP, LYRICS_SCRIPT, "12"),
    "labels.html": (LABELS_STYLES, LABELS_MARKUP, LABELS_SCRIPT, "20"),
}

for fname, (styles, markup, script, zidx) in comps.items():
    content = sub_comp_html(
        comp_id=fname.replace(".html", ""),
        styles=styles,
        markup=markup,
        script=HELPERS_JS + script,
        z_index=zidx,
    )
    with open(os.path.join(BASE, "compositions", fname), "w", encoding="utf-8") as f:
        f.write(content)
    print(f"Wrote {fname}")
...
with open(os.path.join(BASE, "index.html"), "w", encoding="utf-8") as f:
    f.write(HOST_HTML)
print("Wrote host index.html")

# Now do the same for v3
v3_comps = comps.copy()
# v3 orb-trail is included in orb.html
v3_orb_styles = ORB_STYLES + """
    .orb-trail {{
...
for fname, (styles, markup, script, zidx) in v3_comps.items():
    if fname == "orb.html":
        styles = v3_orb_styles
        markup = v3_orb_markup
        script = v3_orb_script
    content = sub_comp_html(
        comp_id=fname.replace(".html", ""),
        styles=styles,
        markup=markup,
    ...
    with open(os.path.join(V3_BASE, "compositions", fname), "w", encoding="utf-8") as f:
        f.write(content)
    print(f"Wrote v3/{fname}")

V3_HOST_HTML = f"""<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=1920, height=1080">
...
with open(os.path.join(V3_BASE, "index.html"), "w", encoding="utf-8") as f:
    f.write(V3_HOST_HTML)
print("Wrote v3 host index.html")
print("Done!")
